class_name ScenesData
extends RefCounted

const SCENE_01_DRAFT: PackedScene = preload("res://scenes/draft/draft_screen.tscn")
const SCENE_02_MARKET: PackedScene = preload("res://scenes/minimarket/mini_market.tscn")
const SCENE_03_BATTLE: PackedScene = preload("res://scenes/minimarket/battle/battle_scene.tscn")
const SCENE_04_SUMMARY: PackedScene = preload("res://scenes/summary/summary_screen.tscn")
